...
export function isTable(furniture: FurnitureItem): boolean {
  return ['round-table', 'square-table', 'rectangular-table', 'booth'].includes(furniture.type);
}
 ...
 */
export function autoLinkChairsToTables(
  furniture: FurnitureItem[],
  linkFunction: (chairId: string, tableId: string) => void,
 ...
 */
export function getSuggestedChairPosition(
  table: FurnitureItem,
  chairIndex: number,
  totalChairs: number,
  chairDimensions: { width: number; height: number }
): { x: number; y: number } {
  const tableCenter = {
