...
export const usePOSRedux = () => {
  ...
  const editingSaleId = useSelector(posSelectors.selectEditingSaleId);
  ...
  const setSelectedSaleForEdit = (sale: any | null) => {
    // Skip if the reference is the same as the current selectedSaleForEdit
    if (sale === selectedSaleForEdit) {
      console.log("🚫 usePOSRedux: Skipping setSelectedSaleForEdit - same reference");
      return;
    }
  ...
  const clearEditingSaleId = () => dispatch(posActions.clearEditingSaleId());
