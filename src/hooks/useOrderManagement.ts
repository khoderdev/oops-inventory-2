...
export const useOrderManagement = () => {
  ...
  const loadOrder = useCallback(async (orderId: string, expectedOrderNumber?: string): Promise<{ order?: Order; error?: string }> => {
    setIsLoading(true);
    setError(null);
    try {
      console.log(`Loading order ID: ${orderId}${expectedOrderNumber ? ` (Expected order number: ${expectedOrderNumber})` : ""}`);

      const response = await ordersAPI.getOrder(orderId);
      console.log("API response for order:", response.data);

      // Handle different response formats
      let order: Order;
      if (response.data && typeof response.data === "object") {
        order = "data" in response.data ? response.data.data : response.data;
      } else {
        throw new Error("Invalid order data format received from server");
      }

      // Validate order number if expected
      if (expectedOrderNumber && order.orderNumber !== expectedOrderNumber) {
        console.warn(`Order number mismatch: expected ${expectedOrderNumber}, got ${order.orderNumber}`);
        // Option 1: Throw an error
        // throw new Error(`Order number mismatch: expected ${expectedOrderNumber}, got ${order.orderNumber}`);

        // Option 2: Include warning in return value
        return {
          order,
          error: `Warning: Order number mismatch (expected ${expectedOrderNumber})`
        };
      }

      console.log("Setting currentOrder:", { id: order.id, orderNumber: order.orderNumber });
      setCurrentOrder(order);
      return { order };
    } catch (error: unknown) {
      const errorMessage = (error as any)?.response?.data?.message || "Failed to load order";
      console.error("Failed to load order:", errorMessage, error);
      setError(errorMessage);
      return { error: errorMessage };
    } finally {
      setIsLoading(false);
