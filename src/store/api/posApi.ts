...
export const posApi = createApi({
  ...
  endpoints: builder => ({
    ...
    getOrders: builder.query<OrderSummary[], { limit?: number; offset?: number; status?: string; orderType?: string }>({
      ...
      providesTags: result => (result ? [...result.map(({ id }) => ({ type: "Orders" as const, id })), { type: "Orders", id: "LIST" }] : [{ type: "Orders", id: "LIST" }]),
      // Keep for 15 minutes
      ...
      onCacheEntryAdded: async (arg, { cacheDataLoaded, cacheEntryRemoved, updateCachedData }) => {
        try {
          ...
          // Only save if orders is defined and is an array
          if (orders && Array.isArray(orders)) {
            ...
            console.warn("⚠️ [posApi] No valid orders data to save to localStorage");
          }
        } catch (e) {
    ...
    getTables: builder.query<Table[], { includeOrders?: boolean; isActive?: boolean }>({
      ...
      providesTags: result => (result ? [...result.map(({ id }) => ({ type: "Tables" as const, id })), { type: "Tables", id: "LIST" }] : [{ type: "Tables", id: "LIST" }]),
      ...
      onCacheEntryAdded: async (arg, { cacheDataLoaded, cacheEntryRemoved, updateCachedData }) => {
        try {
          ...
          // Only save if tables is defined and is an array
          if (tables && Array.isArray(tables)) {
            ...
            console.warn("⚠️ [posApi] No valid tables data to save to localStorage");
          }
        } catch (e) {
    ...
    getFoodMenuItems: builder.query<MenuItem[], boolean>({
      ...
      onCacheEntryAdded: async (arg, { cacheDataLoaded, cacheEntryRemoved, updateCachedData }) => {
        try {
          ...
          // Only save if foodItems is defined and is an array
          if (foodItems && Array.isArray(foodItems)) {
            // Save to localStorage for ultra-fast loading
            localStorage.setItem(
              "pos_cache_food_items",
              JSON.stringify({
                data: foodItems,
                timestamp: Date.now()
              })
            ...
            console.warn("⚠️ [posApi] No valid food menu items data to save to localStorage");
          }
        } catch (e) {
    ...
    getBeverageMenuItems: builder.query<MenuItem[], boolean>({
      ...
      onCacheEntryAdded: async (arg, { cacheDataLoaded, cacheEntryRemoved, updateCachedData }) => {
        try {
          ...
          // Only save if beverageItems is defined and is an array
          if (beverageItems && Array.isArray(beverageItems)) {
            // Save to localStorage for ultra-fast loading
            localStorage.setItem(
              "pos_cache_beverage_items",
              JSON.stringify({
                data: beverageItems,
                timestamp: Date.now()
              })
            ...
            console.warn("⚠️ [posApi] No valid beverage menu items data to save to localStorage");
          }
        } catch (e) {
    ...
    getCategoriesByType: builder.query<Category[], string>({
      ...
      onCacheEntryAdded: async (arg, { cacheDataLoaded, cacheEntryRemoved, updateCachedData }) => {
        try {
          ...
          // Only save if categories is defined and is an array
          if (categories && Array.isArray(categories)) {
            // Save to localStorage for ultra-fast loading
            localStorage.setItem(
              `pos_cache_${arg}_categories`,
              JSON.stringify({
                data: categories,
                timestamp: Date.now()
              })
            ...
            console.warn(`⚠️ [posApi] No valid ${arg} categories data to save to localStorage`);
          }
        } catch (e) {
...
export const {
  endpoints: { getOrders, getTables, getFoodMenuItems, getBeverageMenuItems, getCategoriesByType, getCurrentDayOperation }
} = posApi;
