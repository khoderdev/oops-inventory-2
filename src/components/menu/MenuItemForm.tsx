...
//       // First try to get cost from stock entries
//       const materialStockEntries = stockEntries.filter(entry => entry.materialId === String(ingredient.materialId) && entry.costPerBaseUnit && entry.costPerBaseUnit > 0);
...
//             return {
//               totalCost: acc.totalCost + (entry.costPerBaseUnit || 0) * quantity,
...
//         }
//       } else {
//         // Fallback to material's costPerUnit if no valid stock entries
//         costPerUnit = parseFloat(String(material.costPerUnit || "0"));
//       }

//       // Convert ingredient quantity to base units for final cost calculation
...
//       // Get stock entries for the material
//       const materialStockEntries = stockEntries.filter(entry => entry.materialId === String(ingredient.materialId) && entry.costPerBaseUnit !== null && entry.costPerBaseUnit !== undefined && !isNaN(entry.costPerBaseUnit));

//       let costPerUnit = 0;

//       if (materialStockEntries.length > 0) {
//         console.log(`Stock entries for ${material.name}:`, materialStockEntries);
//         // Calculate weighted average cost from stock entries
//         const { totalCost, totalQuantity } = materialStockEntries.reduce(
//           (acc, entry) => {
//             const quantity = entry.purchasedIndividualQuantity || 0;
...
//         }
//       } else {
//         console.warn(`No valid stock entries for ${material.name}, using fallback cost: 0`);
//       }
...
//     (materialId: string) => {
//       const materialStockEntries = stockEntries.filter(entry => String(entry.materialId) === String(materialId) && entry.costPerBaseUnit !== null && entry.costPerBaseUnit !== undefined && !isNaN(entry.costPerBaseUnit) && entry.costPerBaseUnit > 0);
