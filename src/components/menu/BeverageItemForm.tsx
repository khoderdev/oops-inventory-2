...
export function BeverageItemForm({ menuItem, categories, onSubmit, onCancel }: BeverageItemFormProps) {
  ...
  const handleBeverageSelect = useCallback(
    (beverageId: string, beverageName?: string) => {
      ...
      // If we have a beverage selected, use its name as the menu item name
      if (beverageId) {
    ...
    <div className="space-y-6 p-4">
      <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-8 gap-4">
        ...
        <div className="md:col-span-1 lg:col-span-2">
          ...
          <select id="category" value={normalizedCategory} onChange={e => setCategory(e.target.value as BeverageItemCategory | "")} onKeyDown={handleKeyDown} className="w-full px-3 py-2 border border-input bg-background rounded-md" aria-invalid={!!errors.category} aria-describedby={errors.category ? "category-error" : undefined}>
