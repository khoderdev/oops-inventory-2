...
const LogoPreloader: React.FC<LogoPreloaderProps> = ({ onPreloadComplete, showDebugInfo = false }) => {
  ...
  return (
    <div className="fixed bottom-4 right-4 bg-black/80 text-white p-3 rounded-lg text-xs font-mono z-50">
      ...
      <div className="space-y-1">
        <div>
          Progress: {preloadStatus.completed}/{preloadStatus.total}
        </div>
        ...
        {preloadStatus.errors.length > 0 && <div className="text-red-400 text-xs">{preloadStatus.errors.length} errors</div>}

        <div className="text-gray-400">Cache: {logoCache.getCacheStats().cachedLogos} logos</div>
      </div>
    </div>
  );
};
