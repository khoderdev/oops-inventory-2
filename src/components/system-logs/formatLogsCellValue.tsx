...
export function formatLogsCellValue(
  ...
  if (header === "Quantity Δ") {
    ...
    return (
      <div className={`flex items-center gap-1 ${colorClass} font-medium`}>
        {icon}
        <span>{isPositive ? "+" : ""}{formatNumber(quantity)}</span>
      </div>
    );
  }
  ...
  if (header === "Cost Δ") {
    ...
    return (
      <div className={`flex items-center gap-1 ${colorClass} font-medium`}>
        {icon}
        <span>{isPositive ? "+" : ""}{formatCurrency(cost)}</span>
      </div>
    );
  }
  ...
  if (header === "User") {
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        <span className={`${isSystem ? 'font-medium text-blue-600 dark:text-blue-400' : ''}`}>
          {userName}
        </span>
      </div>
    );
