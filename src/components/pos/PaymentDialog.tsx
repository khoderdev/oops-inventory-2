...
export const PaymentDialog: React.FC<PaymentDialogProps> = ({ isOpen, onClose, total, paymentAmount, onPaymentAmountChange, onPayment, isLoading }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      // Small delay to ensure dialog is fully opened and accessible
      ...
      <DialogContent className="w-full h-full  max-h-none m-0 p-0 bg-gray-50 overflow-hidden border-0 rounded-none max-w-lg">
        <div className="w-full h-full flex flex-col overflow-hidden">
                  ...
                  <div className="flex justify-center">
                    <div className="relative w-full max-w-72">
                      ...
                      <Input 
                        ...
                        tabIndex={isDialogReady ? 0 : -1}
                        className="h-16 pl-12 pr-6 text-emerald-400 text-2xl sm:!text-4xl text-center font-bold border-2 border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 rounded-xl bg-white shadow-lg transition-all duration-200 font-mono w-full" 
                      />
                    </div>
                  </div>
                  ...
                  {quickAmounts.map(amount => {
                    ...
                    return (
                      <Button 
                        ...
                        tabIndex={isDialogReady ? 0 : -1}
                        className={`h-12 sm:h-14 p-2 sm:p-3 text-sm sm:text-base font-bold transition-all duration-300 transform hover:scale-105 hover:shadow-lg ${isSelected ? "bg-gradient-to-r from-primary to-primary hover:from-primary hover:to-primary text-white shadow-lg scale-105" : "border-2 border-primary hover:border-primary"}`}
                      >
          ...
          <DialogFooter className="flex-shrink-0 bg-gradient-to-r from-gray-50 to-slate-50 ">
            <div className="w-full space-y-3 bg-gray-200">
              <div className="flex w-full">
                <Button 
                  ...
                  onClick={onClose} 
                  tabIndex={isDialogReady ? 0 : -1}
                  className="flex-1 h-20 sm:h-20 text-base sm:text-2xl font-bold border-2 border-gray-300 hover:border-red-400 hover:bg-red-50 transition-all duration-200 rounded-none"
                >
                  Cancel
                </Button>
                <Button
                  ...
                  size="lg"
                  tabIndex={isDialogReady ? 0 : -1}
                  className={`flex-1 h-20 sm:h-20 text-base sm:text-2xl font-bold transition-all duration-300 transform ${isButtonDisabled ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-gradient-to-r from-emerald-600 to-emerald-600 hover:from-emerald-700 hover:to-emerald-700 text-white shadow-lg hover:shadow-xl rounded-none"}`}
                >
                  {isLoading ? (
                    <>
                    ...
                    <>
                      <Check className="w-4 h-4 sm:!w-10 sm:!h-10 mr-2" />
              ...
              {paymentValue > 0 && (
                <div className="text-center text-xs sm:text-lg pb-2">
                  <div className="flex items-center justify-center space-x-2 sm:space-x-3 flex-wrap font-mono text-teal-500">
                    <span>
                    ...
                    {!hasChange && (
                      <>
                        <span className="hidden sm:inline">•</span>
                        <span className="text-blue-600 mt-2 sm:mt-0 font-semibold w-full sm:w-auto text-center font-mono">
                          No Change to return
                        </span>
                      </>
                    )}
