...
// const OrderItemsListBase: React.FC<OrderItemsListProps> = ({ cart, updateCartQuantity, orderType, selectedTable, selectedEmployee, onOrderTypeChange, onTableSelect, onEmployeeSelect, incompleteTableOrdersCount, orderStatus, isOrderCompleted, discountReason, leftPanelPixelWidth = 0, onShowItemNotes }) => {
...
//   // Use a stable reference to the cart to prevent re-renders
//   const stableCart = React.useMemo(() => {
//     console.log("🔍 OrderItemsList using cart:", cart?.length || 0, "items");
//     return Array.isArray(cart) ? cart : [];
//   }, [cart]);

//   // Log cart changes but don't use effects that might cause re-renders
//   console.log("🧷 OrderItemsList rendering with cart length:", stableCart.length);
//   if (stableCart.length > 0) {
//     console.log("🧷 OrderItemsList has items");
//   }
...
//         {!stableCart || stableCart.length === 0 ? (
...
const OrderItemsListBase: React.FC<OrderItemsListProps> = ({
  cart,
  updateCartQuantity,
  orderType,
  selectedTable,
  ...
  const handleQuantityUpdate = React.useCallback((cartId: string, newQuantity: number) => {
    if (isCompleted) {
      return;
    }
    updateCartQuantity(cartId, newQuantity);
  }, [isCompleted, updateCartQuantity]);
  ...
  const handleEmployeeSelected = React.useCallback((employee: Employee | null) => {
    if (employee) {
      onEmployeeSelect(employee);
      onOrderTypeChange("employees");
    }
  }, [onEmployeeSelect, onOrderTypeChange]);

  const handleItemClick = React.useCallback((itemId: string) => {
    if (isCompleted) return;
    setSelectedItemForNotes(prev => prev === itemId ? null : itemId);
  }, [isCompleted]);

  const handleItemNotesClick = React.useCallback((e: React.MouseEvent, item: POSCartItem) => {
    e.stopPropagation();
    if (isCompleted || !onShowItemNotes) {
      return;
    }
    onShowItemNotes(item);
  }, [isCompleted, onShowItemNotes]);
    ...
    return (["delivery", "takeaway", "bar", "table", "employees"] as OrderType[]).map(type => {
      ...
      return (
        <Button 
          ...
          size="sm" 
          onClick={handleClick} 
          className={`relative flex items-center justify-center ${shouldShowLabels ? "h-10" : "h-8"} rounded-none transition-all duration-200 ${orderType === type ? "bg-teal-500 hover:bg-teal-600 text-white" : "hover:bg-gray-50"}`}
        >
          <div className="flex items-center justify-center">
            {getOrderTypeIcon(type)}
            {shouldShowLabels && <span className="text-xs font-medium ml-1">{getOrderTypeLabel(type, selectedTable, selectedEmployee, discountReason)}</span>}
  ...
  }, [orderType, selectedTable, selectedEmployee, discountReason, shouldShowLabels, incompleteTableOrdersCount, onTableSelect, handleEmployeeSelectClick, onOrderTypeChange]);
          ...
          <div className="p-4 space-y-3">
            {stableCart.map(item => (
              <div key={item.id} className="space-y-2">
                {/* Main Item Row */}
                <div 
                  className={`flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0 cursor-pointer transition-colors duration-200 rounded-lg px-2 -mx-2 ${selectedItemForNotes === item.id ? "bg-blue-50 border-blue-200" : "hover:bg-gray-50"} ${isCompleted ? "cursor-default" : ""}`} 
