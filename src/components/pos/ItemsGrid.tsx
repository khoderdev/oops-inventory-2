...
export const ItemsGrid: React.FC<ProductGridProps> = ({ posItems, onAddToCart, rightPanelPixelWidth = 0, isLoading = false }) => {
  ...
  const ProductItem: React.FC<{ item: POSItem }> = React.memo(({ item }) => (
    <Card
      className="group relative items-card cursor-pointer select-none transition-all duration-300 hover:shadow-lg hover:shadow-primary/20 hover:-translate-y-1 border border-gray-200 hover:border-primary/40 rounded-lg bg-white/95 backdrop-blur-sm overflow-hidden btn-touch"
      onClick={() => onAddToCart(item)}
      style={{
        height: gridConfig.itemHeight
      }}
    >
      <CardContent className="p-0 h-full flex flex-col">
        ...
        <div className="relative flex-1 w-full overflow-hidden">
          {item.type === "menu_item" && item.image ? (
            <img src={item.image.startsWith("data:") ? item.image : `http://localhost:3000${item.image}`} alt={item.name} className="w-full h-full object-contain transition-all duration-300 group-hover:scale-110" onError={e => handleImageError(e, item.type)} loading="lazy" />
          ) : (
            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary/5 to-primary/10">
          ...
          <div className="absolute top-2 right-2 transform translate-x-8 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300 ease-out">
