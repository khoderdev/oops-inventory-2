  ...
  return (
    <div className="flex-1 p-4 overflow-y-auto">
      <div className="grid gap-5 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 3xl:grid-cols-7">
        {/* Unified POS Items */}
        {posItems.map(item => (
          <Card key={item.id} className="cursor-pointer select-none transition-all hover:shadow-lg hover:scale-105 border-2 border-primary hover:border-primary rounded-lg" onClick={() => onAddToCart(item)}>
