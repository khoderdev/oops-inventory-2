...
import { Grid, Move, ZoomIn, ZoomOut, Link } from 'lucide-react';
...
export const FloorPlanCanvas: React.FC<FloorPlanCanvasProps> = ({
  ...
  const handleMoveWithChairs = useCallback((tableId: string) => {
  ...
  const getAvailableTables = useCallback((): FurnitureItemType[] => {
    return area.furniture.filter(f => 
      ['round-table', 'square-table', 'rectangular-table', 'booth'].includes(f.type)
    );
  }, [area.furniture]);
  ...
  const getCanvasDimensions = () => {
    ...
    const totalWidth = Math.max(
      area.bounds.width * 3,
      scaledViewportWidth + Math.abs(pan.x / scale) + padding
    );
    const totalHeight = Math.max(
      area.bounds.height * 3,
      scaledViewportHeight + Math.abs(pan.y / scale) + padding
    );
    ...
    <defs>
      ...
      >
        <path
          ...
          opacity={Math.min(1, scale * 0.8 + 0.2)}
        />
      </pattern>
    </defs>
  ) : null;
    ...
    <div className="flex-1 bg-gray-50 relative overflow-hidden select-none">
      ...
      <div className="absolute top-2 right-4 z-10 bg-white rounded-lg shadow-lg border border-gray-200 p-2 flex items-center gap-2">
        ...
        <button
          ...
          className={`p-2 rounded-lg transition-colors ${
            showGrid ? 'bg-amber-100 text-amber-700' : 'hover:bg-gray-100'
          }`}
          title="Toggle Grid"
        >
        ...
        <button
          onClick={handleAutoLinkChairs}
          className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
          title="Auto-link chairs to nearby tables"
        >
          ...
          >
            <div className="relative w-full h-full">
              {area.furniture.map((furniture) => (
                <div
                  key={furniture.id}
                  onContextMenu={(e) => handleFurnitureRightClick(furniture, e)}
                >
                  <FurnitureItem
                    furniture={furniture}
      ...
      {contextMenu && (
        <FurnitureContextMenu
          ...
          onLinkToTable={onLinkChairToTable}
          onUnlinkFromTable={onUnlinkChairFromTable}
          onMoveWithChairs={handleMoveWithChairs}
          onDuplicate={onDuplicateFurniture}
          onDelete={onDeleteFurniture}
          availableTables={getAvailableTables()}
          childChairs={getChildFurniture(contextMenu.furniture.id)}
          parentTable={getParentFurniture(contextMenu.furniture.id)}
        />
      )}
    </div>
  );
