...
export const FurnitureItem: React.FC<FurnitureItemProps> = ({ furniture, isSelected, onSelect }) => {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: furniture.id });
  ...
  const getFurnitureShape = () => {
    ...
    const baseClasses = `absolute border-2 transition-all duration-200 hover:shadow-lg ${isSelected ? "border-amber-500 shadow-lg ring-2 ring-amber-200" : "border-gray-300 hover:border-gray-400"}`;
    ...
    switch (furniture.type) {
      ...
      case "chair":
        return (
          <div className={`${baseClasses} rounded-sm`} style={shapeStyle}>
  ...
  return (
    ...
    >
      ...
      <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 bg-slate-800 text-white text-xs px-2 py-1 rounded opacity-0 hover:opacity-100 transition-opacity whitespace-nowrap">{furniture.name}</div>
    </div>
  );
};
