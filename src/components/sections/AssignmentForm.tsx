...
//   const watchedInnerSectionId = form.watch("innerSectionId");
...
//   // Get already assigned items for the selected section and inner section to prevent duplications
//   const assignedStockEntryIds = useMemo(() => {
//     if (!watchedSectionId) return new Set();
//     const stockEntryAssignments = existingAssignments.filter(assignmentItem => assignmentItem.sectionId === watchedSectionId && assignmentItem.innerSectionId === watchedInnerSectionId && assignmentItem.stockEntryId && (!assignment || assignmentItem.id !== assignment.id));
//     return new Set(stockEntryAssignments.map(assignment => assignment.stockEntryId));
//   }, [existingAssignments, watchedSectionId, watchedInnerSectionId, assignment?.id]);

//   const assignedMenuItemIds = useMemo(() => {
//     if (!watchedSectionId) return new Set();
//     const menuItemAssignments = existingAssignments.filter(assignmentItem => assignmentItem.sectionId === watchedSectionId && assignmentItem.innerSectionId === watchedInnerSectionId && assignmentItem.menuItemId && (!assignment || assignmentItem.id !== assignment.id));
//     return new Set(menuItemAssignments.map(assignment => assignment.menuItemId));
//   }, [existingAssignments, watchedSectionId, watchedInnerSectionId, assignment?.id]);
...
//     try {
//       await onAssignAll(watchedSectionId, watchedItemType, availableItemsForAssignAll, watchedInnerSectionId === "none" ? undefined : watchedInnerSectionId);
//     } catch (error) {
...
//     // Validate innerSectionId
//     if (data.innerSectionId && data.innerSectionId !== "none") {
//       const validInnerSection = availableInnerSections.find(innerSection => innerSection.id === data.innerSectionId);
//       if (!validInnerSection) {
//         setFormError("Invalid inner section selected.");
//         return;
//       }
//     }
...
//       }
//       if (data.innerSectionId !== assignment.innerSectionId) {
//         submissionData.innerSectionId = data.innerSectionId === "none" ? undefined : data.innerSectionId;
//       }
...
//         innerSectionId: data.innerSectionId === "none" ? undefined : data.innerSectionId,
...
//               {availableInnerSections.length > 0 && (
//                 <FormField
//                   control={form.control}
//                   name="innerSectionId"
//                   render={({ field }) => (
...
//                               <span className="text-xs text-muted-foreground">Assign directly to section</span>
//                             </div>
//                           </SelectItem>
//                           {availableInnerSections.map(innerSection => (
//                             <SelectItem key={innerSection.id} value={innerSection.id}>
//                               <div className="flex flex-col">
...
export function AssignmentForm({ sections = [], stockEntries = [], materials = [], menuItems = [], assignment, existingAssignments = [], onSubmit, onCancel, isLoading = false, selectedSectionId, onAssignAll }: AssignmentFormProps) {
  ...
  useEffect(() => {
    if (watchedItemType === "stockEntry") {
      ...
      form.setValue("stockEntryId", "");
      form.setValue("assignedQuantity", undefined);
      form.setValue("assignedUnit", "");
    }
  ...
  const assignedStockEntryIds = useMemo(() => {
    if (!watchedSectionId) return new Set();
    const stockEntryAssignments = existingAssignments.filter(assignmentItem => assignmentItem.sectionId === watchedSectionId && (assignmentItem.innerSectionId || "none") === watchedInnerSectionId && assignmentItem.stockEntryId && (!assignment || assignmentItem.id !== assignment.id));
    return new Set(stockEntryAssignments.map(assignment => assignment.stockEntryId));
  }, [existingAssignments, watchedSectionId, watchedInnerSectionId, assignment?.id]);

  const assignedMenuItemIds = useMemo(() => {
    if (!watchedSectionId) return new Set();
    const menuItemAssignments = existingAssignments.filter(assignmentItem => assignmentItem.sectionId === watchedSectionId && (assignmentItem.innerSectionId || "none") === watchedInnerSectionId && assignmentItem.menuItemId && (!assignment || assignmentItem.id !== assignment.id));
    return new Set(menuItemAssignments.map(assignment => assignment.menuItemId));
  }, [existingAssignments, watchedSectionId, watchedInnerSectionId, assignment?.id]);
