    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-4">
              <Button variant={transferType === "full" ? "default" : "outline"} onClick={() => setTransferType("full")} className="flex-1">
                Transfer Entire Order
              </Button>
              <Button variant={transferType === "partial" ? "default" : "outline"} onClick={() => setTransferType("partial")} className="flex-1">
                Transfer Selected Items
              </Button>
            </div>
          </div>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Checkbox checked={selectedItemsCount === totalItems} onCheckedChange={handleSelectAll} />
                ...
                {selectedItemsCount > 0 && (
                  <div className="mt-4 p-3 bg-blue-50 rounded border border-blue-200">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-blue-800">${selectedTotal.toFixed(2)}</span>
                    </div>
                  </div>
                )}
              </CardContent>
                ...
                {availableDestinations.map(table => (
                  <SelectItem key={table.id} value={table.id.toString()}>
                    <div className="flex items-center justify-between w-full">
                      ...
                      <Badge variant={table.status === "opened" ? "destructive" : "secondary"} className="ml-2">
                        {table.status}
                      </Badge>
                    </div>
                  </SelectItem>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleTransfer} disabled={isLoading || !destinationTableId || (transferType === "partial" && selectedItemsCount === 0)} className="flex items-center gap-2">
            {isLoading ? (
              "Transferring..."
            ) : (
              <>
