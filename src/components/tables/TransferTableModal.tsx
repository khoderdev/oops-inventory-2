...
export const TransferTableModal: React.FC<TransferTableModalProps> = ({ isOpen, onClose, onTransferComplete, tables, sourceTable, sourceOrder }) => {
  console.log("🔍 [TransferTableModal] Component render:", {
    isOpen,
    sourceTable,
    sourceOrder,
  ...
  if (!sourceTable || !sourceOrder) {
    console.log("⚠️ [TransferTableModal] Early return - missing data:", {
  ...
  console.log("🎨 [TransferTableModal] RENDERING MODAL - isOpen:", isOpen);
  ...
  return (
    ...
    >
      <div className="space-y-4 sm:space-y-6">
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 mb-3">
              <div className="flex items-center gap-2 min-w-0">
                ...
                {sourceTable.name && <span className="text-sm text-muted-foreground truncate">({sourceTable.name})</span>}
              </div>
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
            <Button variant={transferType === "full" ? "default" : "outline"} onClick={() => setTransferType("full")} className="flex-1 text-sm sm:text-base h-10 sm:h-11">
              <span className="truncate">Transfer Entire Order</span>
            </Button>
            <Button variant={transferType === "partial" ? "default" : "outline"} onClick={() => setTransferType("partial")} className="flex-1 text-sm sm:text-base h-10 sm:h-11">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <div className="relative">
                    <input type="checkbox" checked={selectedItemsCount === totalItems} onChange={e => handleSelectAll(e.target.checked)} className="h-4 w-4 sm:h-5 sm:w-5 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2 cursor-pointer" />
                  </div>
            ...
            <CardContent className="space-y-2 p-3 sm:p-6 pt-0">
              <div className="max-h-60 sm:max-h-80 overflow-y-auto space-y-2">
                {sourceOrder.items.map((item: OrderItem) => {
                  ...
                  return (
                    <div key={item.id} className={`flex items-center justify-between p-2 sm:p-3 border rounded cursor-pointer transition-colors hover:bg-gray-50 ${isSelected ? "bg-blue-50 border-blue-200" : "border-gray-200"}`} onClick={() => handleItemSelection(String(item.id), !isSelected)}>
                      ...
                      <div className="font-medium text-sm sm:text-base flex-shrink-0 ml-2">${item.totalPrice}</div>
                    </div>
                  );
                })}
              </div>
              ...
              {selectedItemsCount > 0 && (
                <div className="mt-3 sm:mt-4 p-3 bg-blue-50 rounded border border-blue-200">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-0">
                    ...
                    <span className="font-bold text-blue-800 text-sm sm:text-base">${selectedTotal.toFixed(2)}</span>
                  </div>
                </div>
              )}
            </CardContent>
              ...
              {availableDestinations.map(table => (
                <SelectItem key={String(table.id)} value={String(table.id)} className="p-2 sm:p-3">
                  <div className="flex items-center justify-between w-full min-w-0">
                    ...
                    <Badge variant={table.status === "opened" ? "destructive" : "secondary"} className="ml-2 text-xs flex-shrink-0">
                      {table.status}
                    </Badge>
                  </div>
                </SelectItem>
            ...
            <AlertDescription>
              <div className="space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-start gap-2">
                  <p className="text-sm sm:text-base flex-1">Destination table has an active order. Choose how to handle the transfer:</p>
                  <TooltipProvider>
                    <Tooltip delayDuration={0}>
                      <TooltipTrigger asChild>
                        <Button variant="ghost" size="sm" className="h-6 w-6 p-0 hover:bg-blue-100 self-start sm:self-center flex-shrink-0" onClick={() => setShowInfoModal(true)}>
                ...
                <div className="flex items-start space-x-2">
                  <div className="relative mt-0.5">
                    <input type="checkbox" checked={createNewOrder} onChange={e => setCreateNewOrder(e.target.checked)} className="h-4 w-4 sm:h-5 sm:w-5 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2 cursor-pointer" />
                  </div>
                  <Label className="text-sm leading-relaxed">Create new order (recommended for separate billing)</Label>
                </div>
      ...
      <div className="mt-6 flex flex-col sm:flex-row gap-2 sm:gap-0 sm:justify-end">
        ...
        <Button onClick={handleTransfer} disabled={isLoading || !destinationTableId || (transferType === "partial" && selectedItemsCount === 0)} className="flex items-center justify-center gap-2 w-full sm:w-auto order-1 sm:order-2">
          {isLoading ? (
            <span className="text-sm sm:text-base">Transferring...</span>
          ) : (
            <>
              <Move className="w-4 h-4 flex-shrink-0" />
              <span className="text-sm sm:text-base truncate">Transfer {transferType === "full" ? "Order" : `${selectedItemsCount} Items`}</span>
            </>
          )}
        </Button>
      </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600">When transferring items to a table that already has an active order, you have two options:</p>
