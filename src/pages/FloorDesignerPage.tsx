...
function FloorDesignerPage() {
  const { currentPlan, selectedFurniture, selectedFurnitureId, setSelectedFurnitureId, addFurniture, updateFurniture, deleteFurniture, duplicateFurniture, newPlan, linkChairToTable, unlinkChairFromTable, getChildFurniture, getParentFurniture, moveTableWithChairs, saveToBackend, loadFromBackend, getSavedPlansFromBackend, deleteFromBackend } = useFloorPlan();
  ...
  const handleGetSavedPlans = useCallback(async () => {
    try {
      return await getSavedPlansFromBackend();
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-gray-50 overflow-hidden">
      <div className="flex-1 flex min-h-0 overflow-hidden">
        <FurniturePalette onAddFurniture={handleAddFurniture} currentPlan={currentPlan} onSavePlan={handleSavePlan} onLoadPlan={handleLoadPlan} getSavedPlans={handleGetSavedPlans} onDeletePlan={handleDeletePlan} onNewPlan={newPlan} />
        ...
        <PropertiesPanel selectedFurniture={selectedFurniture} onUpdateFurniture={updateFurniture} onDeleteFurniture={deleteFurniture} onDuplicateFurniture={duplicateFurniture} onUnselectFurniture={() => setSelectedFurnitureId(null)} parentTable={selectedFurniture ? getParentFurniture(selectedFurniture.id) : null} childChairs={selectedFurniture ? getChildFurniture(selectedFurniture.id) : []} />
      </div>
