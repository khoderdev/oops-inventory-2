          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden sm:table-cell">Opened Date</th>
                <th className="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden sm:table-cell">Closed Date</th>
                  ...
                  <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500 hidden sm:table-cell">
                    {day.openedAt ? (
                      <div className="flex flex-col space-y-1">
                        <div className="text-xs font-medium">{formatDate(day.openedAt)}</div>
                        <div className="text-xs text-blue-600 font-semibold">{new Date(day.openedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })}</div>
                      </div>
                    ) : (
                      <span className="text-xs italic">Not opened</span>
                    )}
                  </td>
                  <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500 hidden sm:table-cell">
                    {day.closedAt ? (
                      <div className="flex flex-col space-y-1">
                        <div className="text-xs font-medium">{formatDate(day.closedAt)}</div>
                        <div className="text-xs text-blue-600 font-semibold">{new Date(day.closedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })}</div>
                      </div>
                    ) : (
                      <span className="text-xs italic">Not closed</span>
                    )}
                  </td>
