...
export function SalesHistoryPage({ isOpen }: { isOpen: boolean; onClose: () => void }) {
  ...
  //       const sale = currentSales.find(s => s.id.toString() === saleId);
  //       if (!sale) {
  //         console.error("Sale not found for editing:", { saleId });
  //         return;
  //       }
  ...
  //       });
  //       console.log("Navigating to /pos with state:", { selectedSaleId: saleId });
  //       // Navigate to POSClient with sale data
  //       navigate("/pos", {
  //         state: {
  //           selectedOrderForPOS: sale
  //         }
  //       });
  //     } catch (error) {
  ...
  //     }
  //   },
  //   [currentSales, navigate]
  // );

  const handleEditSale = useCallback(
    async (saleId: string, orderNumber: string) => {
      // Add orderNumber parameter
      try {
        console.log(`Editing sale: ${saleId}, order number: ${orderNumber}`);

        // Pass both ID and order number for validation
        const { order, error } = await loadOrder(saleId, orderNumber);

        if (error) {
          console.warn(error); // Log warning about order number mismatch
        ...
        if (order) {
          navigate("/pos", {
            state: {
              selectedOrderForPOS: order,
              expectedOrderNumber: orderNumber // Pass expected order number
            }
          });
        }
      } catch (error) {
