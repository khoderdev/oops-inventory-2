...
export function SalesHistoryPage({ isOpen }: { isOpen: boolean; onClose: () => void }) {
  ...
  // Use SalesContext instead of useSalesOperations
  const { sales, staffSales, loading, error, fetchSales, fetchStaffSales, revertSale, softDeleteSale, deleteSaleItem, deleteSaleItems } = useSales();
  ...
  const handleRevertSale = useCallback(
    (saleId: string) => {
      const sale = currentSales.find(s => s.id.toString() === saleId);
      if (sale) {
        console.log("Reverting sale:", { saleId: sale.id, orderNumber: sale.orderNumber });
        setSelectedSaleForRevert(sale);
        setRevertDialogOpen(true);
      }
    },
  ...
  const handleSoftDeleteSale = useCallback(
    (saleId: string, itemId?: string, itemType?: "material" | "menu", itemName?: string) => {
      const sale = currentSales.find(s => s.id.toString() === saleId);
      if (!sale) return;
      console.log("Soft deleting sale:", { saleId, itemId, itemType, itemName });
      if (itemId && itemType) {
        setSelectedItemForDelete({ saleId, itemId, itemType, itemName: itemName || "this item" });
        setDeleteConfirmationModalOpen(true);
      } else {
  ...
  const handleBulkRevert = useCallback(() => {
    console.log("Initiating bulk revert for selected items:", Array.from(selectedItemIds));
    setBulkRevertDialogOpen(true);
  }, [setBulkRevertDialogOpen]);

  const handleBulkDelete = useCallback(() => {
    console.log("Initiating bulk delete for selected items:", Array.from(selectedItemIds));
    setBulkDeleteDialogOpen(true);
  }, [setBulkDeleteDialogOpen]);

  const cancelRevert = useCallback(() => {
    console.log("Cancelling revert action for sale:", selectedSaleForRevert?.id);
    setRevertDialogOpen(false);
    setSelectedSaleForRevert(null);
  }, [setRevertDialogOpen, setSelectedSaleForRevert]);
  ...
  const cancelBulkRevert = useCallback(() => {
    console.log("Cancelling bulk revert action");
    setBulkRevertDialogOpen(false);
  }, [setBulkRevertDialogOpen]);

  const cancelBulkDelete = useCallback(() => {
    console.log("Cancelling bulk delete action");
    setBulkDeleteDialogOpen(false);
  }, [setBulkDeleteDialogOpen]);
  ...
  const confirmSoftDeleteSale = useCallback(async () => {
    try {
      if (selectedItemForDelete) {
        const { saleId, itemId, itemType } = selectedItemForDelete;
        console.log("Confirming soft delete for item:", { saleId, itemId, itemType });
        const sale = sales.find(s => s.id.toString() === saleId);
        if (sale) {
          const totalItems = (sale.items?.length || 0) + (sale.menuItems?.length || 0);
          if (totalItems === 1) {
            console.log("Last item in sale, deleting entire sale:", { saleId });
            await softDeleteSale(sale);
            setDeleteSuccess(`Order #${saleId} has been deleted (last item removed).`);
          } else {
            console.log("Deleting specific item from sale:", { saleId, itemId, itemType });
            await deleteSaleItem(saleId, itemId, itemType);
            setDeleteSuccess(`Item successfully deleted from order #${saleId}`);
          }
        }
  ...
  const confirmBulkRevert = useCallback(async () => {
    if (bulkRevertSales && setBulkRevertDialogOpen) {
      ...
      console.log("Confirming bulk revert for sales:", Array.from(selectedSaleIds));
      await bulkRevertSales(selectedSaleIds);
      setBulkRevertDialogOpen(false);
      clearSelection();
    }
  ...
  const confirmBulkDelete = useCallback(async () => {
    if (deleteSaleItems && setBulkDeleteDialogOpen) {
      ...
      console.log("Confirming bulk delete for sales by type:", salesByType);
      await Promise.all(Object.entries(salesByType).map(([type, ids]) => deleteSaleItems("bulk", ids, type as "material" | "menu")));
  ...
  useEffect(() => {
    if (isOpen) {
      if (viewMode === "all") {
        console.log("Fetching all sales for viewMode:", viewMode);
        fetchSales();
      } else {
        console.log("Fetching staff sales for viewMode:", viewMode);
        fetchStaffSales();
      }
    }
  }, [viewMode, fetchSales, fetchStaffSales, isOpen]);
