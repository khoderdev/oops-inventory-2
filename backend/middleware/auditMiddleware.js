...
const SENSITIVE_FIELDS = ["password", "token", "secret", "key", "authorization", "cookie", "session", "csrf", "api_key", "access_token", "refresh_token", "credit_card", "ssn", "social_security"];
...
const shouldDeduplicateLogout = (userId, sessionId, requestData, responseData) => {
  ...
  // For explicit logout logs from authController, check for duplicates within time window
...
export const auditMiddleware = (options = {}) => {
  ...
  return async (req, res, next) => {
    ...
    if (sessionId) {
      Session.update({ lastActivity: new Date() }, { where: { token: sessionId, isActive: true } }).catch(err => console.error("Failed to update session activity:", err));
    }
