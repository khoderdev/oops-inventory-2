...
const stockEntriesController = {
  ...
  async wasteFromSpecificEntry(req, res, next) {
    try {
      ...
      if ((material.unitType === "mass" || isMassUnit) && (stockEntry.massUnit !== updatedMassUnit || stockEntry.costPerMassUnit !== updatedCostPerMassUnit)) {
  ...
  addToStock: async (req, res, next) => {
    try {
      ...
      // Format each entry with a nested supplier object
      const formattedEntries = createdEntries.map(entry => {
        const entryData = entry.toJSON ? entry.toJSON() : entry;
        return {
          ...entryData,
          supplier: {
